...
function getRelativeTime(d1: Date | string | null | undefined, d2: Date = new Date()): string | undefined {
	// `new Date(null)` is the Unix epoch, which rendered as "Jan 1st, 1970".
	if (d1 === null || d1 === undefined || d1 === '') return undefined;

	if (!(d1 instanceof Date)) d1 = new Date(d1);
	if (isNaN(d1.getTime())) return undefined;
...
function getMonth(dateString: string | null | undefined): string | undefined {
...
function getDate(dateString: string | null | undefined): number | undefined {
	if (!dateString) return undefined;
	const d = new Date(dateString);
	if (isNaN(d.getTime())) return undefined;
	return d.getDate();
...
function getDay(dateString: string | null | undefined): string | undefined {
	if (!dateString) return undefined;
	const d = new Date(dateString);
	if (isNaN(d.getTime())) return undefined;
