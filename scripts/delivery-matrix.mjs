...
const FORBIDDEN_KEYS = /"(employeeId|assigneeId|assigneeName|assignee|createdByName|ownerName|internalNotes|authorUserId|createdById|postedById|uploadedById|actorUserId|email)"\s*:/;
...
async function main() {
	...
	await prisma.project.update({ where: { id: projA.id }, data: { currentStage: 'PLANNING', health: 'ON_TRACK', healthReason: null } });

	// ------------------------------------------------ pure formula
	hr('WEIGHTED PROGRESS FORMULA (unit)');
	check('spec example 100x20 + 50x40 + 25x40 = 50%', weightedProgress([{ progressPercentage: 100, weight: 20 }, { progressPercentage: 50, weight: 40 }, { progressPercentage: 25, weight: 40 }]) === 50, '');
	check('no milestones → null (Not available yet)', weightedProgress([]) === null, '');
	check('zero/negative weights ignored', weightedProgress([{ progressPercentage: 100, weight: 0 }, { progressPercentage: 40, weight: -5 }, { progressPercentage: 20, weight: 10 }]) === 20, '');
	check('all weights invalid → null', weightedProgress([{ progressPercentage: 100, weight: 0 }]) === null, '');
	check('all completed → 100', weightedProgress([{ progressPercentage: 100, weight: 3 }, { progressPercentage: 100, weight: 7 }]) === 100, '');
	check('doc example (design 100/20, fe 70/25, be 60/30, qa 10/15, dep 0/10) = 57', weightedProgress([{ progressPercentage: 100, weight: 20 }, { progressPercentage: 70, weight: 25 }, { progressPercentage: 60, weight: 30 }, { progressPercentage: 10, weight: 15 }, { progressPercentage: 0, weight: 10 }]) === 57, '');
	check('out-of-range progress clamped', weightedProgress([{ progressPercentage: 250, weight: 1 }, { progressPercentage: -50, weight: 1 }]) === 50, '');
	...
	for (const [label, path, opts] of [
		['POST milestones', `/api/admin/projects/${projA.id}/milestones`, { method: 'POST', body: { title: 'x', weight: 1 } }],
		...
		['POST client change request', `/api/portal/projects/${projA.id}/change-requests`, { method: 'POST', body: { type: 'BUG', title: 'x', description: 'y' } }],
	...
	hr('ROLE BOUNDARIES');
	let r = await call(A.j, `/api/admin/projects/${projA.id}/milestones`, { method: 'POST', body: { title: 'x', weight: 1 } });
	...
	r = await call(O.j, `/api/portal/projects/${projA.id}/change-requests`, { method: 'POST', body: { type: 'BUG', title: 'x', description: 'y' } });
	...
	r = await mk({ title: 'UI/UX Design', weight: 20, status: 'COMPLETED', progressPercentage: 30, dueDate: '2026-09-30' });
	...
	r = await mk({ title: 'Backend Development', weight: 40, status: 'IN_PROGRESS', progressPercentage: 50, dueDate: '2026-10-10', clientVisible: true });
	const m2 = r.data?.milestone;
	check('M2 created (50%, w40)', r.ok && m2?.progressPercentage === 50, '');
	r = await mk({ title: 'Internal Refactor', weight: 40, status: 'IN_PROGRESS', progressPercentage: 25, dueDate: '2026-10-05', clientVisible: false });
	...
	check('owner overall progress = 45 (all milestones incl. internal)', r.data?.overallProgress === 45, `${r.data?.overallProgress}%`);
	check('owner next milestone = Backend (nearest due, client-visible)', r.data?.nextMilestone?.title === 'Backend Development', r.data?.nextMilestone?.title);
	check('owner counts', r.data?.counts?.completed === 1 && r.data?.counts?.total === 4, JSON.stringify(r.data?.counts));

	r = await call(O.j, `/api/admin/projects/${projA.id}/milestones/reorder`, { method: 'POST', body: { orderedIds: [m4.id, m3.id, m2.id, m1.id, '000000000000000000000000'] } });
	check('reorder accepted (unknown ids ignored)', r.ok, '');
	r = await call(O.j, `/api/admin/projects/${projA.id}`);
	check('reorder persisted', r.data.milestones.map((m) => m.id).join(',') === [m4.id, m3.id, m2.id, m1.id].join(','), '');

	r = await mk({ milestoneId: m2.id, title: 'Backend Development', weight: 40, status: 'IN_PROGRESS', progressPercentage: 80 });
	...
	r = await call(O.j, `/api/admin/projects/${projB.id}/milestones`, { method: 'POST', body: { milestoneId: m2.id, title: 'Hijack', weight: 1 } });
	check('editing milestone via wrong project → 404', r.status === 404, `HTTP ${r.status}`);

	// ------------------------------------------------ stage / health
	hr('OWNER — STAGE + HEALTH');
	r = await call(O.j, `/api/admin/projects/${projA.id}`, { method: 'PATCH', body: { currentStage: 'DEVELOPMENT', health: 'AT_RISK', healthReason: 'Third-party API delays.' } });
	check('owner sets stage + health', r.ok && r.data.project.currentStage === 'DEVELOPMENT' && r.data.project.health === 'AT_RISK', '');
	...
	check('stage NOT auto-derived from progress (still DEVELOPMENT at 56%)', pA.currentStage === 'DEVELOPMENT', pA.currentStage);

	// ------------------------------------------------ tasks
	hr('OWNER — TASKS');
	const tk = (body) => call(O.j, `/api/admin/projects/${projA.id}/tasks`, { method: 'POST', body });
	r = await tk({ title: 'Catalogue API', functionalTeam: 'DEVELOPMENT', status: 'IN_PROGRESS', progressPercentage: 70, clientVisible: true, milestoneId: m2.id, dueDate: '2026-10-08' });
	const t1 = r.data?.task;
	check('client-visible task created', r.ok && t1?.clientVisible === true, t1?.title);
	r = await tk({ title: 'Refactor auth middleware (internal notes: talk to Rana)', functionalTeam: 'DEVELOPMENT', status: 'IN_PROGRESS', progressPercentage: 20, clientVisible: false, description: 'Assigned to Rana Abdul Rehman' });
	const t2 = r.data?.task;
	check('internal task created', r.ok && t2?.clientVisible === false, '');
	r = await tk({ title: 'QA Testing', functionalTeam: 'QA', status: 'TODO', progressPercentage: 50, clientVisible: true });
	...
	r = await tk({ taskId: t1.id, title: 'Catalogue API', status: 'COMPLETED', clientVisible: true, functionalTeam: 'DEVELOPMENT' });
	check('task completed → progress 100', r.ok && r.data.task.progressPercentage === 100, '');
	r = await tk({ taskId: t1.id, title: 'Catalogue API', status: 'IN_PROGRESS', progressPercentage: 70, clientVisible: true, functionalTeam: 'DEVELOPMENT', milestoneId: m2.id });
	check('task reopened', r.ok && r.data.task.status === 'IN_PROGRESS', '');

	// ------------------------------------------------ updates
	hr('OWNER — UPDATES');
	r = await call(O.j, `/api/admin/projects/${projA.id}/updates`, { method: 'POST', body: { title: 'Development Update', message: 'Authentication complete. Catalogue API under development.', milestoneId: m2.id, clientVisible: true } });
	check('client-visible update posted', r.ok && r.data.update.clientVisible === true, '');
	r = await call(O.j, `/api/admin/projects/${projA.id}/updates`, { method: 'POST', body: { title: 'Internal standup', message: 'Rana is on leave Friday.', clientVisible: false } });
	...
	check('client sees stage + health + reason', ws?.project?.currentStage === 'DEVELOPMENT' && ws?.project?.health === 'AT_RISK' && ws?.project?.healthReason?.includes('Third-party'), '');
	check('client next milestone = Backend', ws?.nextMilestone?.title === 'Backend Development', ws?.nextMilestone?.title);
	check('client sees only client-visible milestones (3 of 4)', ws?.milestones?.length === 3 && !ws.milestones.some((m) => m.id === m3.id), `${ws?.milestones?.length}`);
	check('client milestone has no weight/clientVisible/org fields', ws?.milestones?.every((m) => !('weight' in m) && !('organizationId' in m) && !('clientVisible' in m)), '');
	check('client sees only client-visible tasks (2 of 3)', ws?.tasks?.length === 2 && !ws.tasks.some((t) => t.id === t2.id), `${ws?.tasks?.length}`);
	check('client task has no description/priority/org fields', ws?.tasks?.every((t) => !('description' in t) && !('priority' in t) && !('organizationId' in t)), '');
	check('client task carries functional team, not a person', ws?.tasks?.[0]?.functionalTeam === 'DEVELOPMENT', ws?.tasks?.[0]?.functionalTeam);
	check('client sees only client-visible updates (1 of 2)', ws?.updates?.length === 1 && ws.updates[0].title === 'Development Update', `${ws?.updates?.length}`);
	check('internal text never appears in client payload', !wsJson.includes('Rana') && !wsJson.includes('standup') && !wsJson.includes('Refactor'), '');
	check('client payload has no forbidden identity keys', !FORBIDDEN_KEYS.test(wsJson), (wsJson.match(FORBIDDEN_KEYS) || [''])[0]);
	...
	const before = await prisma.project.findUnique({ where: { id: projA.id } });
	...
	r = await call(A.j, `/api/portal/projects/${projA.id}/change-requests`, { method: 'POST', body: { type: 'NEW_FEATURE', title: 'x', description: 'y', relatedMilestoneId: m3.id } });
	check('client cannot reference an INTERNAL milestone', r.status === 400, `HTTP ${r.status}`);
	r = await call(A.j, `/api/portal/projects/${projA.id}/change-requests`, { method: 'POST', body: { type: 'HACK', title: 'x', description: 'y' } });
	check('invalid type rejected', r.status === 400, `HTTP ${r.status}`);
	r = await call(A.j, `/api/portal/projects/${projB.id}/change-requests`, { method: 'POST', body: { type: 'BUG', title: 'x', description: 'y' } });
	...
	const tr = (to, note) => call(O.j, `/api/admin/change-requests/${cr.id}/transition`, { method: 'POST', body: { to, note } });
	r = await call(O.j, '/api/admin/change-requests');
	check('owner inbox lists the request with client + project', r.ok && r.data.changeRequests.some((c) => c.id === cr.id && c.organization?.name === 'Client A Ltd' && c.project?.name), '');
	r = await tr('ACCEPTED');
	...
	check('UNDER_REVIEW → CLARIFICATION_REQUIRED (with question)', r.ok && r.data.changeRequest.status === 'CLARIFICATION_REQUIRED', '');
	r = await tr('ACCEPTED');
	check('CLARIFICATION_REQUIRED → ACCEPTED refused', r.status === 409, `HTTP ${r.status}`);

	r = await call(A.j, `/api/portal/projects/${projA.id}/workspace`);
	check('client sees action required: clarify', r.data?.actionsRequired?.some((a) => a.key === `cr-${cr.id}`), '');
	check('client sees Eiretech question in thread', r.data?.changeRequests?.[0]?.messages?.some((m) => m.authorSide === 'EIRETECH' && m.body.includes('Which reports')), '');
	...
	r = await call(A.j, `/api/portal/change-requests/${cr.id}/reply`, { method: 'POST', body: { body: 'Sales and Inventory.' } });
	...
	check('IN_PROGRESS → COMPLETED', r.ok && r.data.changeRequest.status === 'COMPLETED' && !!r.data.changeRequest.resolvedAt, '');
	r = await tr('UNDER_REVIEW');
	...
	check('client thread has only EIRETECH/CLIENT labels', msgs.every((m) => ['EIRETECH', 'CLIENT'].includes(m.authorSide)), '');
	check('client CR payload has no identity keys', !FORBIDDEN_KEYS.test(crJson), (crJson.match(FORBIDDEN_KEYS) || [''])[0]);
	check('client sees resolution note', r.data?.changeRequests?.[0]?.resolutionNote?.includes('shipped'), '');
	check('client action required cleared after completion', !r.data?.actionsRequired?.some((a) => a.key === `cr-${cr.id}`), '');
	...
	const tamper = [
		['PATCH project stage/health/progress', `/api/admin/projects/${projA.id}`, { method: 'PATCH', body: { currentStage: 'COMPLETED', health: 'ON_TRACK', overallProgress: 100 } }],
		['POST milestone progress/status', `/api/admin/projects/${projA.id}/milestones`, { method: 'POST', body: { milestoneId: m2.id, title: 'x', weight: 1, progressPercentage: 100, status: 'COMPLETED', clientVisible: true } }],
		['DELETE milestone', `/api/admin/projects/${projA.id}/milestones/${m2.id}`, { method: 'DELETE' }],
		['POST task progress/status', `/api/admin/projects/${projA.id}/tasks`, { method: 'POST', body: { taskId: t1.id, title: 'x', status: 'COMPLETED', progressPercentage: 100 } }],
		['POST internal task made visible', `/api/admin/projects/${projA.id}/tasks`, { method: 'POST', body: { taskId: t2.id, title: 'x', clientVisible: true } }],
		['POST project update', `/api/admin/projects/${projA.id}/updates`, { method: 'POST', body: { title: 'x', message: 'y' } }],
	];
	...
	check('DB: project unchanged after tampering', s1.project.currentStage === s0.project.currentStage && s1.project.health === s0.project.health, '');
	check('DB: milestone unchanged after tampering', s1.milestone.progressPercentage === s0.milestone.progressPercentage && s1.milestone.status === s0.milestone.status, '');
	check('DB: task unchanged after tampering', s1.task.progressPercentage === s0.task.progressPercentage && s1.task.status === s0.task.status, '');
	...
	r = await call(A.j, `/api/portal/change-requests/${cr.id}/reply`, { method: 'POST', body: { body: 'x', status: 'ACCEPTED', organizationId: orgB.id } });
	const crAfter = await prisma.changeRequest.findUnique({ where: { id: cr.id } });
	check('reply body with status/org fields cannot change them', crAfter.status === 'COMPLETED' && crAfter.organizationId === orgA.id, '');
	...
	for (const t of ['MILESTONE_CREATED', 'MILESTONE_UPDATED', 'PROJECT_UPDATE_POSTED', 'PROJECT_STAGE_CHANGED', 'PROJECT_HEALTH_CHANGED', 'CHANGE_REQUEST_UNDER_REVIEW', 'CHANGE_REQUEST_CLARIFICATION_REQUIRED', 'CHANGE_REQUEST_ACCEPTED', 'CHANGE_REQUEST_IN_PROGRESS', 'CHANGE_REQUEST_COMPLETED', 'CHANGE_REQUEST_MESSAGE']) {
	...
	check('owner notified of CR submission + reply', r.data?.notifications?.some((n) => n.type === 'CHANGE_REQUEST_SUBMITTED') && r.data.notifications.some((n) => n.type === 'CHANGE_REQUEST_MESSAGE'), '');
	...
	// ------------------------------------------------ audit + activity
	hr('AUDIT + ACTIVITY');
	const actions = new Set((await prisma.auditLog.findMany({ take: 300, orderBy: { createdAt: 'desc' } })).map((a) => a.action));
	for (const a of ['MILESTONE_CREATED', 'MILESTONE_UPDATED', 'MILESTONES_REORDERED', 'PROJECT_TASK_CREATED', 'PROJECT_TASK_UPDATED', 'PROJECT_TASK_COMPLETED', 'PROJECT_UPDATE_POSTED', 'PROJECT_STAGE_CHANGED', 'PROJECT_HEALTH_CHANGED', 'CHANGE_REQUEST_CREATED', 'CHANGE_REQUEST_REVIEW_STARTED', 'CHANGE_REQUEST_CLARIFICATION_REQUESTED', 'CHANGE_REQUEST_REPLIED', 'CHANGE_REQUEST_ACCEPTED', 'CHANGE_REQUEST_STARTED', 'CHANGE_REQUEST_COMPLETED', 'CHANGE_REQUEST_ATTACHMENT_UPLOADED']) {
		check(`audit: ${a}`, actions.has(a), '');
	}
	const leaky = (await prisma.auditLog.findMany({ take: 300 })).filter((a) => JSON.stringify(a.metadata ?? {}).match(/password|secret|token|storageKey/i));
	check('no secrets in audit metadata', leaky.length === 0, `${leaky.length}`);
	r = await call(O.j, `/api/admin/projects/${projA.id}`);
	check('owner activity includes milestone/task/CR events', r.data.activity.some((a) => a.action === 'MILESTONE_CREATED') && r.data.activity.some((a) => a.action === 'CHANGE_REQUEST_COMPLETED'), `${r.data.activity.length} entries`);
	check('owner files tab lists CR attachment', r.data.files.some((f) => f.id === att.id && f.url === `/api/cr-attachments/${att.id}`), '');
