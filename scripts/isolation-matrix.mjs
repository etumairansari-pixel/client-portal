...
async function buildFixture(O, C, org, project, tag) {
	...
	const m = await call(O.j, `${base}/milestones`, {
		method: 'POST',
		body: { title: `${tag} milestone`, weight: 10, status: 'IN_PROGRESS', progressPercentage: 40, clientVisible: true },
	});
	const mi = await call(O.j, `${base}/milestones`, {
		method: 'POST',
		body: { title: `${tag} internal milestone`, weight: 10, clientVisible: false },
	});
	const t = await call(O.j, `${base}/tasks`, { method: 'POST', body: { title: `${tag} task`, clientVisible: true } });
	const ti = await call(O.j, `${base}/tasks`, {
		method: 'POST',
		body: { title: `${tag} internal task`, clientVisible: false },
	});
	const u = await call(O.j, `${base}/updates`, {
		method: 'POST',
		body: { title: `${tag} update`, message: `${tag} progress note`, clientVisible: true },
	});
	const cr = await call(C.j, `/api/portal/projects/${project.id}/change-requests`, {
		method: 'POST',
		body: { type: 'BUG', title: `${tag} request`, description: `${tag} description` },
	});
	const att = await call(C.j, `/api/portal/change-requests/${cr.data?.changeRequest?.id ?? cr.data?.id}/attachments`, {
		method: 'POST',
		raw: pdf(`${tag}.pdf`),
	});
	...
	await call(O.j, `/api/admin/change-requests/${crId}/transition`, {
		method: 'POST',
		body: { to: 'CLARIFICATION_REQUIRED', note: `${tag} clarify?` },
	});
...
async function probe(label, me, other, otherOrg) {
	...
	r.cr = await call(me.j, `/api/portal/projects/${other.project}/change-requests`, {
		method: 'POST',
		body: { type: 'BUG', title: 'x', description: 'y' },
	});
	check('raise change request on their project', r.cr.status === 404, `HTTP ${r.cr.status}`);
	r.crm = await call(me.j, `/api/portal/projects/${me.ids.project}/change-requests`, {
		method: 'POST',
		body: { type: 'BUG', title: 'x', description: 'y', relatedMilestoneId: other.milestone },
	});
	check('reference their milestone from own project', r.crm.status === 400, `HTTP ${r.crm.status}`);
	r.reply = await call(me.j, `/api/portal/change-requests/${other.changeRequest}/reply`, {
		method: 'POST',
		body: { body: 'hello' },
	});
	check('reply on their change request', r.reply.status === 404, `HTTP ${r.reply.status}`);
	r.att = await call(me.j, `/api/portal/change-requests/${other.changeRequest}/attachments`, {
		method: 'POST',
		raw: pdf('x.pdf'),
	});
	...
	for (const [name, path, opts] of [
		...
		[
			'admin milestone reorder',
			`${adminBase}/milestones/reorder`,
			{ method: 'POST', body: { orderedIds: [other.milestone] } },
		],
		['admin update post', `${adminBase}/updates`, { method: 'POST', body: { title: 'x', message: 'y' } }],
		[
			'admin CR transition',
			`/api/admin/change-requests/${other.changeRequest}/transition`,
			{ method: 'POST', body: { to: 'ACCEPTED' } },
		],
		[
			'admin CR reply',
			`/api/admin/change-requests/${other.changeRequest}/reply`,
	...
	check(
		'tampered create: accepted with own org/project only',
		!!row && row.organizationId === me.user.organizationId && row.projectId === me.ids.project,
		'',
	);
...
async function main() {
	...
	check(
		'owner inbox lists A and B requests',
		titles.includes('A request') && titles.includes('B request'),
		`${list.length} total`,
	);
	...
	check(
		'no cross-tenant messages on A',
		(await prisma.changeRequestMessage.count({
			where: { changeRequestId: A.ids.changeRequest, authorUserId: B.user.id },
		})) === 0,
		'',
	);
	check(
		'no cross-tenant messages on B',
		(await prisma.changeRequestMessage.count({
			where: { changeRequestId: B.ids.changeRequest, authorUserId: A.user.id },
		})) === 0,
		'',
	);
	check(
		'no cross-tenant attachments',
		(await prisma.changeRequestAttachment.count({
			where: {
				OR: [
					{ changeRequestId: A.ids.changeRequest, organizationId: orgB.id },
					{ changeRequestId: B.ids.changeRequest, organizationId: orgA.id },
				],
			},
		})) === 0,
		'',
